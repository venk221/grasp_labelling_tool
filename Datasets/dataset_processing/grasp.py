...
class GraspRectangles:
    ...
    def draw(self, shape, position=True, angle=True, width=True):
        """
        Plot all GraspRectangles as solid rectangles in a numpy array.
        """
        if position:
        ...
        for gr in self.grs:
            ...
            if position:
                pos_out[rr, cc] = 1.0
            if angle:
                # ang_out[rr, cc] = gr.angle  # Changed from gr.angle() to gr.angle
                angle_val = gr.angle
                cos_out = np.cos(2 * angle_val)  
                sin_out = np.sin(2 * angle_val)
                
                cos_out = np.clip(cos_out, -1.0, 1.0)
                sin_out = np.clip(sin_out, -1.0, 1.0)
                angle_val = np.arctan2(sin_out, cos_out) / 2  
                
                ang_out[rr, cc] = angle_val
    ...
    def to_array(self, pad_to=0):
        ...
        """
        a = np.stack([gr.points for gr in self.grs])   #changed from gr.points() to gr.points
        if pad_to:
    ...
    def center(self):
        ...
        """
        points = [gr._points for gr in self.grs]  # Changed from gr.points() to gr._points
...
class GraspRectangle:
    ...
    @property
    def points(self):  # Changed from method to property
        """
        :return: Array of points in rectangle
        """
        return self._points
    ...
    def angle(self):
        ...
        """
        dx = self._points[1, 1] - self._points[0, 1]
        dy = self._points[1, 0] - self._points[0, 0]
    ...
    def center(self):
        ...
        """
        return self._points.mean(axis=0).astype(np.int64)
    ...
    def width(self):
        ...
        """
        dy = self._points[2, 1] - self._points[1, 1]
        dx = self._points[2, 0] - self._points[1, 0]
        return np.sqrt(dx ** 2 + dy ** 2)
    ...
    def polygon_coords(self, shape=None):
        ...
        """
        return polygon(self._points[:, 0], self._points[:, 1], shape)
    ...
    def iou(self, gr, angle_threshold=np.pi/6):
        ...
        rr2, cc2 = polygon(gr._points[:, 0], gr._points[:, 1])
    ...
    def copy(self):
        ...
        :return: Copy of self.
        """
        return GraspRectangle(self._points.copy())
    ...
    def offset(self, offset):
        ...
        """
        self._points += np.array(offset).reshape((1, 2))
    ...
    def rotate(self, angle, center):
        ...
        self._points = ((np.dot(R, (self._points - c).T)).T + c).astype(np.int64)
    ...
    def plot(self, ax, color=None):
        ...
        """
        points = np.vstack((self._points, self._points[0]))
    ...
    def zoom(self, factor, center):
        ...
        c = np.array(center).reshape((1, 2))
        self._points = ((np.dot(T, (self._points - c).T)).T + c).astype(np.int64)
...
def detect_grasps(q_img, ang_img, width_img=None, no_grasps=2):
    """
    ...
    """
    # print(f"q_img in detect_grasps - min: {q_img.min()}, max: {q_img.max()}")
    local_max = peak_local_max(q_img, min_distance=20, threshold_abs=0.2, num_peaks=no_grasps)
    # local_max = peak_local_max(q_img, min_distance=5, threshold_abs=0.01, num_peaks=no_grasps)
