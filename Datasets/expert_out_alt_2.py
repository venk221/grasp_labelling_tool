...
class ExpertOutDataset(Dataset):
    def __init__(self, expert_out_dirs, experts = None,start=0.0, end=1.0):
        ...
        # Blacklist experts if requested
        self.__blacklisted_experts = []
        if experts is not None:
            self.__blacklisted_experts = [i for i, expert in enumerate(self.__expert_names) if expert not in experts]
            print(f'Blacklisting experts {self.__blacklisted_experts}')
        ...
        print(f'rgergev = {len(os.listdir(self.__expert_out_dirs)) - 7}')
        for idx in range(len(os.listdir(self.__expert_out_dirs)) - 7):
            idx = str(idx)
            # Store them here
            ...
            for expert_id in range(len(glob.glob(os.path.join(self.__expert_out_dirs, idx, '*q.npy')))):
                if expert_id in self.__blacklisted_experts:
                    continue # Skip blacklisted experts
    ...
    def __getitem__(self, idx):
        ...
        return expert_tensor, y, idx, 0, 0   #"""CHANGE THIS TO RETURN THE GRASP DATA"""
