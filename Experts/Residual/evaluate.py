...
def parse_args():
    ...
    parser.add_argument('--input-size', type=int, default=(300, 300),
    ...
    parser.add_argument('--start', type=float, default=0.9,
                        help='Start of the dataset split')
    parser.add_argument('--end', type=float, default=1.0,
                        help='End of the dataset split')
    ...
    parser.add_argument('--out_dir', type=str,
                        help='Output directory of eval results')
...
if __name__ == '__main__':
    ...
    start_split = int(np.floor(args.start * len(test_dataset)))
    end_split = int(np.floor(args.end * len(test_dataset)))
    if args.ds_shuffle:
        np.random.seed(args.random_seed)
        np.random.shuffle(indices)
    val_indices = indices[start_split:end_split]
    ...
    for network in args.network:
        ...
        with torch.no_grad():
            for idx, (x, y, didx, rot, zoom) in enumerate(test_data):
                ...
                if args.iou_eval:
                    s = evaluation.calculate_iou_match(q_img, ang_img, test_data.dataset.get_grasps(didx),
