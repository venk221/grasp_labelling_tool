...
class GraspRectDispConfig(object):
    ...
    gripper_open_hovering_pen = QPen(gripper_open_color, line_hovering_width)
    gripper_open_selected_pen = QPen(gripper_open_selected_color, line_selected_width)


class GraspRectBuilder(object):
    def __init__(self):
        self.points = np.zeros((4, 2), dtype=np.float64)
    ...
    def processPoint(self, pos: QPointF, button: Qt.MouseButton):
        ...
        point = np.array([pos.x(), pos.y()], dtype=np.float64)
...
class GraspRect(object):
    ...
    def __init__(self, points: np.ndarray = None):
        ...
        if points is None or not isinstance(points, (list, np.ndarray)) or len(points) < 4:
            print(f"[WARNING] Invalid points provided for GraspRect id={self._id}. Using default points.")
            self._points = np.array([[0, 0], [10, 0], [10, 10], [0, 10]], dtype=np.float64)  # Default rectangle
        else:
            points = np.array(points, dtype=np.float64)
            if points.shape != (4, 2):
                print(f"[WARNING] Points shape {points.shape} invalid for GraspRect id={self._id}. Using default points.")
                self._points = np.array([[0, 0], [10, 0], [10, 10], [0, 10]], dtype=np.float64)
            else:
                self._points = points

        # Compute grasp and edges based on validated points
        self._grasp = self.computeGraspFromPoints(self._points)
        self._points = self.computePointsFromGrasp(self._grasp)  # Ensure points align with grasp
    ...
    @classmethod
    def from_model_output(cls, center, gripper_open, gripper_size, angle, confidence=1.0):
        """
        Create a GraspRect from model output.
        
        Args:
            center (np.ndarray): [x, y] coordinates of grasp center.
            gripper_open (float): Width of the grasp (maps to gripper_open).
            gripper_size (float): Height of the grasp (maps to gripper_size).
            angle (float): Grasp angle in radians.
            confidence (float): Model confidence score (optional).
        
        Returns:
            GraspRect: A new GraspRect instance.
        """
        grasp = Grasp(center, gripper_size, gripper_open, angle)
        instance = cls(points=None)
        instance.setGrasp(grasp)
        instance._confidence = confidence
        return instance
    ...
    def getNearestEdge(self, pos: QPointF):
        out_point = np.array([pos.x(), pos.y()], dtype=np.float64)
    ...
    def movePointUpdate(self, select_idx, new_pos: QPointF):
        ...
        new_point = np.array([new_pos.x(), new_pos.y()], dtype=np.float64)
    ...
    def moveWholeUpdate(self, pos: QPointF):
        ...
        delta_pos = np.array([pos.x() - self._pre_cursor_pos.x(),
                              pos.y() - self._pre_cursor_pos.y()], dtype=np.float64)
    ...
    def rotate_degrees(self, angle_degrees):
        if self._points is None or len(self._points) == 0 or self._points.shape[0] < 4:
            print(f"[ERROR] Invalid points for rotation in GraspRect id={self._id}: {self._points}")
            return
        ...
        centered_points = self._points - center
        rotated_points = np.dot(centered_points, rotation_matrix.T) + center
        self._points = rotated_points
        self._grasp = self.computeGraspFromPoints(self._points)  # Update grasp
        self._edges = self.computeEdgesFromPoints(self._points)  # Update edges
        self._area_changed = True
    ...
    def getFillColor(self) -> QColor:
        ...
        if hasattr(self, "_confidence"):  # Model-proposed grasp
            fill_color = QColor.fromHslF(0.33, 1.0, 0.5, alpha)  # Green for model proposals
        else:
            hue = int(self._grasp.angle * 180. / math.pi) % 180 * 2
            fill_color = QColor.fromHslF(hue / 360., 1., 0.5, alpha)  # Angle-based for manual
        return fill_color
    ...
    def export(self):
        ...
        return {
            ...
            "confidence": getattr(self, "_confidence", 1.0)  # Include confidence if available
        }