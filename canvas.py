...
class Canvas(QWidget):
    shapesAdded = pyqtSignal(list)
    shapesRemoved = pyqtSignal(list)
    shapesSelectionChanged = pyqtSignal(list, list)
    shapesAreaChanged = pyqtSignal(list)
    gradientPointSelected = pyqtSignal(QPointF)
    CREATE, EDIT, GRADIENT = 0, 1, 2
    ...
    def rotateSelectedGrasps(self, clockwise=True):
        modified_shapes = []
        for shape in self.shapes:
            if shape.selected():
                angle = 1.0 if clockwise else -1.0  # Rotate by 1 degree
                shape.rotate_degrees(angle)  # Assumes GraspRect has a rotate_degrees method
                modified_shapes.append(shape)
        if modified_shapes:
            self.shapesAreaChanged.emit(modified_shapes)
            self.update()
    ...
    def mousePressEvent(self, e: QMouseEvent) -> None:
        ...
        if self.mode == self.CREATE:
            ...
            if e.button() == Qt.LeftButton:
                print(f"[INFO] Gradient Mode: Mouse click at widget pos={pos}, painter pos={painter_pos}")
                self.gradientPointSelected.emit(painter_pos)
        self._setShapeCursorPos(painter_pos)
        self.pre_pos = pos
        self.update()
    ...
    def setMode(self, mode):
        ...
        elif mode == self.GRADIENT:
            self.mode = self.GRADIENT
            self.builder.reset()
            for shape in self.shapes:
                shape.resetSelected()
                shape.resetHovering()
            if self.underMouse():
                QApplication.changeOverrideCursor(QCursor(Qt.PointingHandCursor))
        self._checkShapesSelectionChangeAndEmit()
        self.update()
    ...
    def enterEvent(self, e: QEvent):
        cursor = QCursor(Qt.CrossCursor) if self.mode == self.CREATE \
            else QCursor(Qt.PointingHandCursor) if self.mode == self.GRADIENT \
