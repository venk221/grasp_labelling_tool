...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        inference_action = action.new_action(
            self, "&Grasp Inference", self.run_model_inference, "Ctrl+G", "inference"
        )
    ...
    def run_model_inference(self):
        ...
        if os.path.exists(depth_path):
            ...
            stacked = np.concatenate([rgb, depth], axis=-1)  # shape (H, W, 4)
        else:
            stacked = rgb
        ...
        # Add grasps to canvas
        gui_grasps = []
        for g in predicted_grasps:
            # Print the original grasp coordinates for debugging
            print(f"[INFO] Predicted grasp: {g.points}")
            
            # Convert coordinates: swap x and y to match GUI coordinate system
            points = np.copy(g.points)
            
            # Create a GraspRect with the corrected coordinates
            try:
                # IMPORTANT FIX: Swap x and y coordinates to match GUI expectations
                # The model outputs points in (row, col) format, but GUI expects (x, y)
                swapped_points = np.zeros_like(points)
                swapped_points[:, 0] = points[:, 1]  # x = original column
                swapped_points[:, 1] = points[:, 0]  # y = original row
        ...
        # Switch to edit mode for better interaction
